...
class FlipFlop:
    ...
    def evaluateMimic(self, problem, color_list):
        ...
        i = 0
        for pop_size_value in pop_size_list:
            _, best_fitness, curve = mlrose.mimic(problem, pop_size=pop_size_value, max_attempts=50, max_iters=100, curve=True, random_state=self.RANDOM_SEED)
            ax.plot(curve, linewidth=8-i*2, label='pop_size='+str(pop_size_value), alpha=0.7, color=color_list[i])
            i = i + 1
    ...
    def comparePerformance(self, problem, color_list, size):
        ...
        self.init_custom_fitness()
        time_begin = time()
        _, best_fitness, curve = mlrose.genetic_alg(problem, pop_size=100, mutation_prob=0.5, max_attempts=self.TOP_MAX_ATTEMPTS, curve=True, random_state=self.RANDOM_SEED)
        time_end = time()
        ...
        self.init_custom_fitness()
        time_begin = time()
        _, best_fitness, curve = mlrose.mimic(problem, pop_size=200, keep_pct=0.6, max_attempts=self.TOP_MAX_ATTEMPTS, curve=True, random_state=self.RANDOM_SEED)
        time_end = time()
        ...
        fig = plt.figure(1)
        plt.title('{title} - Comparison between Algorithms at problem size {size}'.format(title=self.title, size=size))
        plt.xlabel('Iteration')
        plt.ylabel("Fitness")
        plt.legend()
    ...
    def run(self):
        ...
        self.comparePerformance(problem, color_list, 100)

        # size vs performance at size 10
        problem = mlrose.DiscreteOpt(10, fitness)
        self.comparePerformance(problem, color_list, 10)

        # size vs performance at size 20
        problem = mlrose.DiscreteOpt(20, fitness)
        self.comparePerformance(problem, color_list, 20)

        # size vs performance at size 30
        problem = mlrose.DiscreteOpt(30, fitness)
        self.comparePerformance(problem, color_list, 30)
